...
class ProfilesSnapshotTest {
    ...
    fun `reports every connection sharing one routing profile`() {
        ...
        val profileOne = TestFixtures.sampleProfile().copy(id = "profile-1", name = "Hostkey", routingProfileId = sharedRoutingProfile.id)
        val profileTwo = TestFixtures.sampleProfile().copy(id = "profile-2", name = "Exoscale", routingProfileId = sharedRoutingProfile.id)
        val profileThree = TestFixtures.sampleProfile().copy(id = "profile-3", name = "AWS", routingProfileId = "routing-other")
