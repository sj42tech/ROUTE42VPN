...
source_config="${EXOSCALE_SOURCE_CONFIG:-${repo_root}/secrets/PROXY/xray/config.exoscale.json}"
...
backup_config="${backup_dir}/config.json.before-exoscale-switch-${timestamp}"
...
expected_exoscale_ip="$(read_config_address "${source_config}")"
...
echo "Exoscale source config validated successfully."

temp_config="$(mktemp /tmp/xray-exoscale-cutover.XXXXXX.json)"
temp_log="$(mktemp /tmp/xray-exoscale-cutover.XXXXXX.log)"
...
echo "Starting Exoscale canary on ports ${canary_socks_port}/${canary_http_port}..."
...
if ! run_proxy_health_checks "${canary_http_port}" "${canary_socks_port}" "${expected_exoscale_ip}"; then
  echo "Exoscale canary checks failed. Nothing was changed in ${target_config}." >&2
  exit 1
fi
echo "Exoscale canary checks passed."
...
To perform the real cutover with automatic rollback on failure:
  ALLOW_LIVE_SWITCH=1 ${repo_root}/ops/switch-to-exoscale.sh
EOF
  exit 3
fi
...
if restart_launchctl_service "${expected_exoscale_ip}"; then
  cat <<EOF
Exoscale cutover completed.

Active server:
  ${expected_exoscale_ip}
...
if hard_restart_launchctl_service "${expected_exoscale_ip}"; then
  cat <<EOF
Exoscale cutover completed after hard restart.
